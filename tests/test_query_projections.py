...
class TestQueryProjections(unittest.TestCase):
    ...
    def test_actor_projection_copilot_pty_detects_composer_ready_state(self) -> None:
        from cccc.daemon.actors.actor_ops import handle_actor_list
        from cccc.kernel.actors import add_actor
        from cccc.kernel.group import create_group, load_group
        from cccc.kernel.registry import load_registry
        ...
        try:
            with tempfile.TemporaryDirectory() as td:
                ...
                group = load_group(gid)
                self.assertIsNotNone(group)
                add_actor(group, actor_id="peer1", title="Peer One", runtime="copilot", runner="pty")  # type: ignore[arg-type]
                group.save()  # type: ignore[union-attr]

                with (
                    patch("cccc.daemon.actors.actor_ops.pty_runner.SUPERVISOR.actor_running", return_value=True),
                    patch("cccc.daemon.actors.actor_ops.pty_runner.SUPERVISOR.idle_seconds", return_value=420.0),
                    patch(
                        "cccc.daemon.actors.actor_ops.pty_runner.SUPERVISOR.tail_output",
                        return_value=(
                            "[cccc] user → copilot-1: hello\n"
                            "[cccc] If you respond: use MCP ...\n"
                            "@ files · # issues\n"
                            "GPT-5.4\n"
                        ).encode("utf-8"),
                    ),
                ...
                actors = (resp.result or {}).get("actors", [])
                self.assertEqual(actors[0]["effective_working_state"], "idle")
                self.assertEqual(actors[0]["effective_working_reason"], "pty_terminal_copilot_composer_ready")
